...
const webPreferences: WebPreferences = {
  ...
  preload: "", // 脚本路径为文件的绝对路径。 在页面运行其他脚本之前预先加载指定的脚本 无论页面是否集成Node, 此脚本都可以访问所有Node API  当 node integration 关闭时, 预加载的脚本将从全局范围重新引入node的全局引用标志
  ...
  zoomFactor: 1, // 页面的默认缩放系数, 3.0 表示 300%。 默认值为 1.0.
  javascript: true, // 是否启用 JavaScript 支持。 默认值为 true
  // 是否关闭警告信息，electron12之后要将其设置false才能集成node，否则true加载预加载脚本
  ...
  plugins: false, // 是否应该启用插件。
  experimentalFeatures: false, // 启用 Chromium 的实验功能。 默认值为 false.
  ...
  // safeDialogsMessage: "", // 当持续对话框保护被触发时显示的消息。 如果没有定义,就使用默认值
  ...
  enablePreferredSizeMode: false, // 是否启用首选大小模式。 首选大小是包含文档布局所需的最小大小--无需滚动。 启用该属性将导致在首选大小发生变化时，在WebContents 上触发 preferred-size-changed 事件。 默认值为 false.
};

const browserWindowProp: BrowserWindowConstructorOptions = {
  show: true, // 是否启动时展示
  // frame: true, // false:让桌面应用没有边框，这样菜单栏也会消失
  ...
  // trafficLightPosition: //  Point 对象 设置控制按钮在无边框窗口中的位置。
  paintWhenInitiallyHidden: true, //  当show为false并且渲染器刚刚被创建时，它是否应激活。 为了让document.visibilityState 在show: false的情况下第一次加载时正确地工作，你应该把这个设置成false. 设置为 false 将会导致ready-to-show 事件不触发。 默认值为 true。
  ...
  icon: iconPath, // 应用运行时的标题栏图标及任务栏图标,默认使用可执行文件的图标.
  ...
  kiosk: false, // 窗口是否进入kiosk模式。 默认值为 false.
  ...
  titleBarOverlay: false, //  Object | Boolean (可选) - 当在 macOS 使用无框窗口结合 win.setWindowButtonVisibility(true) 或使用 titleBarStyle 以便标准窗口控制 (在 macOS为 "traffic lights") 可见，此属性将启用 Window Controls Overlay JavaScript APIs 和 CSS Environment Variables. 指定 true 将导致覆盖默认系统颜色。 默认值为 false.color String (可选) Windows - 启用窗口控制时覆盖面的 CSS 颜色 默认是系统颜色。 symbolColor String (可选) Windows - 启用时窗口控制中符号的 CSS 颜色 默认是系统颜色。
